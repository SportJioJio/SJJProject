...
router.post("/create", async (req, res, next) => {
  try {
    ...
    if (result.rowCount == 0) res.send({ result: "no user in this group found", status: "fail" });
      ...
      const sql = `INSERT into posts (name, start_time, end_time, max_num, place, launcher_id, group_id) 
                   values ('${name}', '${startTime}', '${endTime}', ${maxNum}, '${place}', '${userId}', '${groupId}') RETURNING id`;
      console.log(sql);
